...
public class VowelsRUsDebug {
	
	 // Enable console logging for debugging (user configurable)
			...
			// If suffix starts with a consonant, drop the leftmost letter of the final sequence of vowels, then add the suffix
				...
				for (int i=word.length(); i>0; i--) {
					...
					if (wordCharacter.matches(vowels)) {
						if (debug == true) System.out.println("[D] Word Character #" + i + ", " + wordCharacter + " is a vowel.");
					}
					else {
						if (debug == true) System.out.println("[D] Word Character #" + i + ", " + wordCharacter + " is a consonant.");
